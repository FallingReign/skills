...
function parseArgs(argv) {
  ...
  if (keywords.length === 0) {
    throw new Error("Usage: node arch-lookup.js [--architecture path] [--text] <feature keywords>");
  }
...
function buildMacroOverview(cells) {
...
function computeBm25Scores(queryTerms, cells) {
...
function buildGreenfieldResult(query, architecturePath, macroOverview, reason, topMatches = []) {
  return {
    query,
    architecturePath,
    score: 0,
